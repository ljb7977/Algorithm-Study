...
void process_instruction(){
	...
	CURRENT_STATE.IF_PC = CURRENT_STATE.IF_ID_NPC;
	CURRENT_STATE.BRANCH_PC = CURRENT_STATE.MEM_WB_BR_TARGET;
	CURRENT_STATE.JUMP_PC = CURRENT_STATE.ID_EX_JUMP_TARGET;
...
void MEM_Stage()
{
	...
	index = (CURRENT_STATE.EX_MEM_ALU_OUT & 0x7) >> 2;
	...
	if(CURRENT_STATE.EX_MEM_MemWrite){ //sw
		if((pt = Cache_Search(CURRENT_STATE.EX_MEM_ALU_OUT)) == 0){
			pt = Cache_Evict(CURRENT_STATE.EX_MEM_ALU_OUT);
			STALL_COUNT = miss_penalty;
		}
		pt[index] = CURRENT_STATE.EX_MEM_W_VALUE;
	}
	...
	if(CURRENT_STATE.EX_MEM_MemRead){ //lw
		...
		CURRENT_STATE.MEM_WB_MEM_OUT = pt[index];
	}
